...
from .basic_modules import ARCH_REGISTRY
from .basic_modules import to_2tuple, trunc_normal_
...
# 这段代码实现了一种称为“随机深度”(Stochastic Depth)的技术，通过丢弃（或跳过）神经网络中的某些层来减少计算量和防止过拟合。
...
class PSA(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        # define a parameter table of relative position bias 相对位置嵌入检索表
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * self.permuted_window_size[0] - 1) * (2 * self.permuted_window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        # get pair-wise aligned relative position index for each token inside the window 相对位置索引，作为下标检索相对位置嵌入检索表
    ...
    def forward(self, x, mask=None):
        ...
        x = self.proj_drop(x)
        return x
    # flops方法用于计算模型的浮点操作数，有助于评估模型的计算复杂度，并不是内置函数；而extra_repr方法用于提供模块的额外描述信息，是nn.Module的内置函数
...
class PSA_Block(nn.Module):
    ...
    def __init__(self,
        ...
        self.permuted_window_size = window_size//2  # 缩小因子为2
    ...
    def calculate_mask(self, x_size):
        ...
        return attn_mask    # 得到的mask应该是长方形的，长为宽的4倍，对应注意力图像的尺寸
...
# 隔2取点，在通道维拼接，再进行通道减半（实际上尺寸缩小为1/4，通道数增大为2倍）
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = sx = x.transpose(1, 2).view(x.shape[0], self.dim, x_size[0], x_size[1])
        return x
...
class PSA_Group(nn.Module):
    ...
    # 执行BasicLayer -> Unembed -> conv -> Embed
    def forward(self, x, x_size):
...
class SRFormer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        return x
    # 将图片填充，变得能被window_size整除
    def check_image_size(self, x):
        _, _, h, w = x.size()
        mod_pad_h = (self.window_size - h % self.window_size) % self.window_size
        mod_pad_w = (self.window_size - w % self.window_size) % self.window_size
        # reflect 指定了填充的模式是反射模式
    ...
    def forward(self, x):
        ...
        x = self.conv_last(self.conv_before_upsample(x))
